...
class beyondDipolePulse:public Potential<beyondDipolePulse> {
	...
	public:
		...
		bdpsqft axialPartSq_impl(std::integral_constant<axis,axis::t> c, double t, int l) const {
			...
			switch(l%2) {
				case 0:
					out(0,0) =  3./32. * sgn * ((Scalar) E0/(pow(omega,2)) * (l==0)								   		      );
					out(1,0) =  1./16. * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar) (4 * PI / T)           ));
					out(2,0) = -1./4.  * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar) (2 * PI / T)           ));
					out(3,0) =  1./32. * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar) (4 * PI / T - 2* omega)));
					out(4,0) = -1./8.  * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar) (2 * PI / T - 2* omega)));
					out(5,0) =  3./16. * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar)             - 2* omega ));
					out(6,0) = -1./8.  * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar) (2 * PI / T + 2* omega)));
					out(7,0) =  1./32. * sgn * ((Scalar) E0/(pow(omega,2)) * cos((phi + t) * (Scalar) (4 * PI / T + 2* omega)));
					break;
				case 1:
					out(0,0) =  0																				   		       ;
					out(1,0) =  1./16. * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar) (4 * PI / T)           ));
					out(2,0) = -1./4.  * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar) (2 * PI / T)           ));
					out(3,0) =  1./32. * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar) (4 * PI / T - 2* omega)));
					out(4,0) = -1./8.  * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar) (2 * PI / T - 2* omega)));
					out(5,0) =  3./16. * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar)             - 2* omega ));
					out(6,0) = -1./8.  * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar) (2 * PI / T + 2* omega)));
					out(7,0) =  1./32. * sgn * ((Scalar) E0/(pow(omega,2)) * sin((phi + t) * (Scalar) (4 * PI / T + 2* omega)));
					break;
			}
			//cout << out;
		...
		bdpsqft axialPartSq_impl(std::integral_constant<axis,axis::radial> c, const Eigen::MatrixBase<MatrixType>& x,int l) const {
			...
			out.row(0) = MatrixType::Constant(x.size(),6./32.) * (l==0);
