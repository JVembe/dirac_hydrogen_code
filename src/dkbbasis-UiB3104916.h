...
class dkbbasis: public basis<dkbbasis>, public splineHandler {
	...
	public:
		...
		csmat& get0mat(int ul = 0) {
			if(ul==-1) return mats0[M][1];
			return mats0[M][0]; 
		}
		
		template<matname M>		
		csmat& getk0mat(int ul = 0) {
			if(ul==-1) return matsk0[M][1];
			return matsk0[M][0];
		}
		
		template<matname M>
		csmat& get0kmat(int ul = 0) {
			if(ul==-1) return mats0k[M][1];
			return mats0k[M][0];
		}
		template<matname M>
		...
		void matmat_impl(std::integral_constant<matname,matname::H0> c, const cmat& m, cmat& w, cvec params) {
			...
			w.noalias() += ((H0_k2 * m).array().rowwise() * kappas.cast<cdouble>().transpose().pow(2)).matrix();
			w.noalias() += ((H0_k3 * m).array().rowwise() * kappas.cast<cdouble>().transpose().pow(3)).matrix();
		...
		void matmat_impl(std::integral_constant<matname,matname::dpa> c, const cmat& m, cmat& w, const csmat& thmat, const cvec& params, int l, int ul) {
			...
			// cout << "m dims: (" << m.rows() << ", " << m.cols() << ")\n";
			// cout << "w dimws: (" << m.rows() << ", " << m.cols() << ")\n";
			// cout << "kappasmat dims: (" << kappasmat.rows() << ", " << kappasmat.cols() << ")\n";
			
			// cout << "thmat dims: (" << thmat.rows() << ", " << thmat.cols() << ")\n";
			
			csmat reducedThmat = angidmat.transpose() * thmat * angidmat;
			
			// cout << "reducedThmat dims: (" << reducedThmat.rows() << ", " << reducedThmat.cols() << ")\n";
			
			// w.transposeInPlace();

			if(ul ==-1) {
				w.noalias() += reducedThmat * (dpalph0 * m).transpose();
				w.noalias() += kappasmat * reducedThmat * (dpalphk0 * m).transpose();
				w.noalias() += reducedThmat * kappasmat * (dpalph0k * m).transpose();
				w.noalias() += kappasmat * reducedThmat * kappasmat * (dpalphkk * m).transpose();
			}
			...
			for(int pp = 0; pp < dkbpart::Npts; pp++) {
				for(int alpha = 0; alpha < 6; alpha++) {
					if(l%2 != alpha%2) {
						...
						bdpalph[1][pp][l] += csmat((pT(alpha,0).real() * dpalph).adjoint());
				...
				if(abs(abs(ik(cacheId) + 0.5) - kappamax - 0.5) <= l <= abs(ik(cacheId) + 0.5) + kappamax + 0.5) {
					...
					if(!isCached(vcache00)) {
						...
						if(ul==-1) {
							...
							vcachek0 = (bdpalph[1][1][l] * v.segment(cacheId*radqN(),radqN()));
							vcache0k = (bdpalph[1][2][l] * v.segment(cacheId*radqN(),radqN()));
							vcachekk = (bdpalph[1][3][l] * v.segment(cacheId*radqN(),radqN()));
						}
						if(ul==1) {
							vcache00 = (bdpalph[0][0][l] * v.segment(cacheId*radqN(),radqN()));
							vcachek0 = (bdpalph[0][1][l] * v.segment(cacheId*radqN(),radqN()));
							vcache0k = (bdpalph[0][2][l] * v.segment(cacheId*radqN(),radqN()));
