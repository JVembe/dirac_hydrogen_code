...
template <typename SchrodingerType, typename basistype>
class SchrodingerBase: public Hamiltonian<SchrodingerType, basistype> {
	friend class Hamiltonian<SchrodingerType, basistype >;
	
	
	public:
		long double (*extPot)(long double);
		SchrodingerBase(basis<basistype>& bs):Hamiltonian<SchrodingerType, basistype>(bs) { };
		
		csmat& H0mat() const {
			if(isCached(this->H0m)) return *(this->H0m);
			else {
				*(this->H0m) = this->bs->psqm() + this->bs->Em(this->Vfunc);
		...
		cvec H0vec(const cvec& v) const {
			if(!isCached(this->bs->getRadial().getH0mat(0)))
				return this->bs->template matfree<psq>(v) + (this->bs->template matfree<E>(v));
			else
				return this->bs->template matfree<H0>(v);
		}
		
		csmat H0radmat() const {
			// if(!isCached(this->bs->getRadial().H0m())) {
				// this->H0radprep();
				// // return this->bs->getRadial().psqm() + this->bs->getRadial().Em(this->Vfunc);
		...
		void H0radprep() {
			//Separate H0(radial) into part that depends on l  and part independent of l
			csmat H0_0 = -0.5 *  this->bs->getRadial().ddm(2) + this->bs->getRadial().Em(this->Vfunc);
			csmat H0_l = this->bs->getRadial().Lsqm();
			
			this->bs->getRadial().getH0mat(0) = H0_0.selfadjointView<Eigen::Upper>();
			this->bs->getRadial().getH0mat(1) = H0_l.selfadjointView<Eigen::Upper>();
		}
		...
		void prepeigs(int nev, int ncv, bool angSep = true) {
			...
			if(!angSep){
				...
				this->eigvecs[0] = this->geigs.eigenvectors().sparseView();
				this->eigvals[0] = this->geigs.eigenvalues();
			}
			else {
				//Separate H0 and S by angular quantum number
				int Nr = this->bs->radqN();
				...
				std::vector<mat> levecs;
				std::vector<vec> levals;
				std::vector<int> ls;
				
				
				int lmax = this->angMax();
				...
				for(int l = 0; l < lmax; l++) {
					...
					this->bs->getRadial().setState(this->getBasis().indexTransform(i));
					...
					vec evals = this->geigs.eigenvalues().real();
					
					//cout << "evll" << l << "\n" << evals << "\n";
					
					
					levals.push_back(evals);
					int eigN = evals.size();
					
					levecs.push_back(this->geigs.eigenvectors().real());
					
					
				}
				
				for(int k = 0; k < levecs.size(); k++) {
					for(int i = 0; i < levecs[k].cols(); i++) {
						//Determine if nonzero 
						double vmax = abs(levecs[k].col(i).maxCoeff());
						double vmin = abs(levecs[k].col(i).minCoeff());
						...
						int j = 0;
						while(true) {
							if(abs(levecs[k](j,i)*100) > vv) {
								break;
							}
							j++;
						}
						
						if(levecs[k](j,i) < 0) levecs[k].col(i) = -levecs[k].col(i);
					}
				}
				...
				for(int j = 0; j < Nang; j++) {
					...
					int l = il(this->getBasis().indexTransform(j));
					...
					//cout << "idx: " << idx << "\n";
					//cout << "idxtfrm: " << this->getBasis().indexTransform(j) << "\n";
					...
					*/
					dsmat eigstates =  (imat * levecs[idx].real()).sparseView();
					...
					this->eigvals[j] = vec(levals[idx].real());
...
class Schrodinger: public SchrodingerBase<Schrodinger<basistype>,basistype>  {
	double* prevt = new double;
	public:
		long double (*extPot)(long double);
		Schrodinger(basis<basistype>& bs):SchrodingerBase<Schrodinger<basistype>, basistype>(bs) { };
		Schrodinger(basis<basistype>& bs, long double (*extPot)(long double)):SchrodingerBase<Schrodinger<basistype>, basistype>(bs) {
				this->extPot = extPot;
		}
		
		
		csmat Htmat(double t) {
			csmat pzm = this->bs->pzm();
			double Et = extPot(t);
		...
		cvec HtVec(double t, const cvec& v) const {
			...
			return Et * this->bs->template matfree<pz>(v);
		}
};

template <typename basistype>
class SchrodingerBDP: public SchrodingerBase<SchrodingerBDP<basistype>, basistype> {
	protected:
		//bdpft (*extPotT) (long double);
		Potential<beyondDipolePulse>* vExt = NULL;
	public:
		SchrodingerBDP(basis<basistype>& bs, Potential<beyondDipolePulse>& vExt):SchrodingerBase<SchrodingerBDP<basistype>,basistype>(bs) {
			this->vExt = &vExt;
		}
		
		csmat Htmat(double t) {
			
			return this->bs->bdpApzm();
		};
		
		cvec HtVec(double t, const cvec& v) const {
			vExt->setTime(t);
			return this->bs->template matfree<bdpApz>(v) + 0.5 * this->bs->template matfree<bdpAsq>(v);
		}
};
...
//Approximation of beyond-dipole as discovered by Esther
template <typename basistype>
class DiracEsther: public DiracBase<DiracEsther<basistype>, basistype> {
	friend class DiracBase<DiracEsther<basistype>,basistype>;
	
	protected:
		Potential<beyondDipolePulse>* vExt = NULL;
		
	public:
		DiracEsther(basis<basistype>& bs, Potential<beyondDipolePulse>& vExt):DiracBase<DiracEsther<basistype>,basistype>(bs) {
			this->vExt = &vExt;
		}
};
#endif